...
public class GateJob {
    ...
    public static void main(String[] args) {
        ...
        // 1. 执行
        DataStream<ZGMessage> execute = GateJob.execute(env,configProperties, parallelism);

        // 2. 主流输出到下游
        CustomKafkaSink.addCustomKafkaSink(execute.map(ZGMessage::getRawData), configProperties.getProperty("kafka.id.sourceTopic"),
    ...
    public static DataStream<ZGMessage> execute(StreamExecutionEnvironment env,Properties configProperties,int parallelism) {
        ...
        // 3.处理数据
        SingleOutputStreamOperator<ZGMessage> gateStep1 = sourceStream.flatMap(new GateFlatMapFunction(configProperties)).setParallelism(parallelism);
        ...
        SingleOutputStreamOperator<ZGMessage> gateStep2 = gateStep1.process(new GateProcessFunction(dataQualityTag4gate, debugTag4gate));

        // 4.1 侧输出 - 数据质量
        DataStream<String> sideOutput4dataQuality = gateStep2.getSideOutput(dataQualityTag4gate);
        ...
        // 4.2 侧输出 - debug
        DataStream<String> sideOutput4debug = gateStep2.getSideOutput(debugTag4gate);
