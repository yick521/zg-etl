...
public class CheckJsonAndOwnerOperator extends RichMapFunction<ZGMessage, ZGMessage> {
    private static final Logger LOG = LoggerFactory.getLogger(CheckJsonAndOwnerOperator.class);

    @Override
    public ZGMessage map(ZGMessage zgMessage) throws Exception {
        LOG.debug("zgMessage : {}", zgMessage);
        try {
            // 尝试将原始数据解析为Map
            ...
            zgMessage.setErrorDescribe(ErrorMessageEnum.BASIC_SCHEMA_FORMAT_NOT_MATCH.getErrorMessage());
            zgMessage.setErrData(zgMessage.getData());
            LOG.debug("CheckJsonAndOwnerOperator map Exception , zgMessage is {} , Exception is  {}",zgMessage,e.getMessage()+" "+e.getLocalizedMessage());
        }
        return zgMessage;
    }
}