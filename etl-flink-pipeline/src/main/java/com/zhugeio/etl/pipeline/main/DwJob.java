...
public class DwJob {
    ...
    private static void addDorisSinks(SingleOutputStreamOperator<EventAttrRow> routed) {
        ...
        DorisRecordSerializer<UserRow> userSerializer = JsonSerializerFactory.createDefaultSerializer();
        DorisRecordSerializer<DeviceRow> deviceSerializer = JsonSerializerFactory.createDefaultSerializer();
        DorisRecordSerializer<UserPropertyRow> userPropSerializer = JsonSerializerFactory.createDefaultSerializer();
        DorisRecordSerializer<EventAttrRow> eventSerializer = JsonSerializerFactory.createDefaultSerializer();

        DorisSinkBuilder.<UserRow>builder()
                .feNodes(feNodes).database(database).table("b_user")
                .username(username).password(password)
                .partialUpdate().serializer(userSerializer).onSuccess(callback)
                .addTo(userStream);

        DorisSinkBuilder.<DeviceRow>builder()
                .feNodes(feNodes).database(database).table("b_device")
                .username(username).password(password)
                .partialUpdate().serializer(deviceSerializer).onSuccess(callback)
                .addTo(deviceStream);

        DorisSinkBuilder.<UserPropertyRow>builder()
                .feNodes(feNodes).database(database).table("b_user_property")
                .username(username).password(password)
                .partialUpdate().serializer(userPropSerializer).onSuccess(callback)
                .addTo(userPropStream);

        DorisSinkBuilder.<EventAttrRow>builder()
                .feNodes(feNodes).database(database).table("b_user_event_attr")
                .username(username).password(password)
