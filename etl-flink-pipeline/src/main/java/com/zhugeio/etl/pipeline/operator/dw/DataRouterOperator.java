...
public class DataRouterOperator extends RichAsyncFunction<ZGMessage, RouterOutput> implements CheckpointedFunction {
    ...
    /**
     *  修复: 异步处理消息 - 返回 CompletableFuture，无阻塞
     */
    private CompletableFuture<RouterOutput> processMessageAsync(ZGMessage message) {
        // 异步版本检查 (非阻塞)
        configCacheService.checkVersionAsync();

        Map<String, Object> msgData = message.getData();
        if (msgData == null) {
            metrics.skip();
            return CompletableFuture.completedFuture(RouterOutput.empty());
        }
        ...
        //  修复: 异步预加载关键词，然后链式处理
        final String finalBusiness = business;
        final String finalDeviceMd5 = deviceMd5;
        String rawJson = message.getRawData();

        return preloadKeywordsAsync(appId, dataList)
                .thenApply(eqidKeywords -> {
                    // 构建输出
                    ...
                    for (Object item : dataList) {
                        ...
                        routeByDt(output, dt, appId, platform, pr, ip, ipResult, ua, uaResult,
                                finalBusiness, eqidKeywords, msgSt, finalDeviceMd5, sdk, pl, rawJson);
                    }
    ...
    /**
     *  修复: 异步预加载百度关键词 - 返回 CompletableFuture，无阻塞
     */
    private CompletableFuture<Map<String, String>> preloadKeywordsAsync(Integer appId, List<?> dataList) {
        ...
        //  修复: 直接返回 Future，不调用 .join()
        return keywordService.preloadKeywordsAsync(eqids)
                .exceptionally(ex -> {
                    LOG.warn("预加载关键词失败", ex);
                    return new HashMap<>();
                });
    }
