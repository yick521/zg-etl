...
public class UserTransfer implements Serializable {
    ...
    public UserRow transferFromMap(Integer appId, Integer platform, Map<String, Object> pr) {
        LOG.info("[UserTransfer] 开始转换: appId={}, platform={}, pr={}", appId, platform, pr);

        if (pr == null || appId == null) {
            LOG.warn("[UserTransfer] pr或appId为空, 跳过");
            return null;
        }
        ...
        LOG.info("[UserTransfer] 提取字段: zgDid={}, zgZgid={}", zgDid, zgZgid);

        // 校验必要字段
        if (isNullOrEmpty(zgDid) || isNullOrEmpty(zgZgid)) {
            LOG.warn("[UserTransfer] zgDid或zgZgid为空, 跳过: zgDid={}, zgZgid={}", zgDid, zgZgid);
            return null;
        }
        ...
        LOG.info("[UserTransfer] 时间字段: ct={}, tz={}", ct, tz);

        if (ct == null || tz == null) {
            LOG.warn("[UserTransfer] ct或tz为空, 跳过");
            return null;
        }

        String time = timestampToDateString(ct, tz);
        LOG.info("[UserTransfer] 转换后的时间: time={}", time);

        if (isNullOrEmpty(time)) {
            LOG.warn("[UserTransfer] 时间转换失败, 跳过");
            return null;
        }
        ...
        LOG.info("[UserTransfer] 创建UserRow成功: deviceId={}, zgId={}, userId={}, beginDate={}, platform={}",
                row.getDeviceId(), row.getZgId(), row.getUserId(), row.getBeginDate(), row.getPlatform());
