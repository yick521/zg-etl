...
public class EventAttrAsyncOperator extends RichAsyncFunction<ZGMessage, ZGMessage> {
    ...
    public void asyncInvoke(ZGMessage input, ResultFuture<ZGMessage> resultFuture) {
        metrics.in();
        LOG.debug("asyncInvoke start input: {}",input);
        ...
        try {
            ...
            LOG.debug("appId={} ,defaultOwner={} ,sdk={} ", appId, defaultOwner, sdk);
    ...
    private void processEventAttrs(ZGMessage input, Integer appId, String defaultOwner,
        ...
        for (Map<String, Object> dataItem : dataList) {
            ...
            for (String attrName : new HashSet<>(pr.keySet())) {
                ...
                LOG.debug("realAttrName={}", realAttrName);
        ...
        LOG.debug("收集到的 allAttrs={}", allAttrs);
        if (allAttrs.isEmpty()) {
            metrics.out();
            resultFuture.complete(Collections.singleton(input));
            return;
    ...
    private CompletableFuture<Map<String, AttrResult>> batchGetOrCreateEventAttrs(List<AttrInfo> attrs, Integer sdk) {
        ...
        LOG.debug("收集到的 groupedByEvent={}", groupedByEvent);
        ...
        for (Map.Entry<Integer, List<AttrInfo>> entry : groupedByEvent.entrySet()) {
            ...
            LOG.debug("循环 eventId={}, eventAttrs={}", eventId,eventAttrs);
    ...
    private CompletableFuture<Void> batchGetOrCreateForEvent(Integer eventId,
        ...
        for (AttrInfo attr : eventAttrs) {
            LOG.debug("循环 AttrInfo attr={}", attr);
            CompletableFuture<Void> cf = cacheService
                    .getEventAttrId(attr.getAppId(), attr.getEventId(),attr.getOwner(), attr.getAttrName())
                    .thenCompose(attrId -> {
                        LOG.debug("从缓存中查询attr attrId={}", attrId);
                        if (attrId != null) {
                            // 缓存命中，异步获取列号
                            return cacheService
                                    .getEventAttrColumn(attr.getEventId(), attrId)
