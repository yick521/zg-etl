...
public class KvrocksClient implements Serializable {
    ...
    /**
     * 异步批量 HMGET (批量获取多个字段值)
     *
     * @param key    Hash key
     * @param fields 要获取的字段列表
     * @return 字段值列表（与 fields 顺序一致，不存在的字段返回 null）
     */
    public CompletableFuture<List<String>> asyncHMGet(String key, List<String> fields) {
        ...
        try {
            ...
            if (isCluster) {
                return getClusterConnection().async().hmget(key, fieldArray)
                        .toCompletableFuture()
                        .thenApply(kvList -> {
                            List<String> result = new ArrayList<>(kvList.size());
                        ...
                        .exceptionally(ex -> {
                            LOG.error("KVRocks HMGET失败: {}, fields={}, {}", key, fields.size(), ex.getMessage());
                            return Collections.emptyList();
                        });
            } else {
                return getStandaloneConnection().async().hmget(key, fieldArray)
                        .toCompletableFuture()
                        .thenApply(kvList -> {
                            List<String> result = new ArrayList<>(kvList.size());
                            for (KeyValue<String, String> kv : kvList) {
            ...
            LOG.error("KVRocks HMGET异常: {}", key, e);
            return CompletableFuture.completedFuture(Collections.emptyList());
        }
    }
    ...
    public List<String> hmGet(String key, List<String> fields) {
        ...
        try {
            String[] fieldArray = fields.toArray(new String[0]);
            List<KeyValue<String, String>> kvList;

            if (isCluster) {
                kvList = getClusterConnection().sync().hmget(key, fieldArray);
            } else {
                kvList = getStandaloneConnection().sync().hmget(key, fieldArray);
            }
            ...
            LOG.error("KVRocks sync HMGET失败: {}, fields={}, {}", key, fields.size(), e.getMessage());
            return Collections.emptyList();
        }
    }
