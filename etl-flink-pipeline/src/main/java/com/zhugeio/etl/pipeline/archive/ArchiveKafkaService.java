...
public class ArchiveKafkaService implements Serializable {
        ...
        if (INSTANCE == null) {
            synchronized (LOCK) {
                if (INSTANCE == null) {
                    ...
                    INSTANCE.init(
                            Config.getString(Config.ID_ARCHIVE_BROKERS),
                            Config.getString(Config.KAFKA_ID_ARCHIVE_TOPIC),
                            Config.getBoolean(Config.ID_ARCHIVE_ENABLED, true)
                    );
                }
            }
        }
    ...
    public void sendToKafka(ArchiveType type, Integer appId,String key,Long value) {
        LOG.debug("[ArchiveKafkaService] sendToKafka type: {} ,appId :{} ,key :{} ,value :{}", type, appId, key, value);
    ...
    public void sendToKafka(String message) {
        LOG.debug("[ArchiveKafkaService] Kafka 发送: {}", message);
        if (producer != null && !closed) {
            producer.send(new ProducerRecord<>(kafkaTopic,  message), (metadata, exception) -> {
                if (exception != null) {
                    LOG.warn("[ArchiveKafkaService] Kafka 发送失败: {}", exception.getMessage());
                }
            });
        }
    }
    ...
    public void close() {
        ...
        if (producer != null) {
            try {
                producer.flush();
                producer.close();
                LOG.info("[ArchiveKafkaService] 已关闭");
            } catch (Exception e) {
                LOG.warn("[ArchiveKafkaService] 关闭失败: {}", e.getMessage());
            }
        }
