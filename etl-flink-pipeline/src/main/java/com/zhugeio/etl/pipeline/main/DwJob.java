 ...
 *
 * 支持两种写入模式:
 * - STREAMING（默认）: 流式写入，依赖 Checkpoint，支持 Exactly-Once
 * - BATCH: 批量写入，不依赖 Checkpoint，高吞吐，适合历史数据回刷
 *
 * 通过配置 doris.sink.write.mode=BATCH 切换为批量模式
 */
public class DwJob {
    ...
    public static SingleOutputStreamOperator<EventAttrRow> execute(DataStream<ZGMessage> source) {
        ...
        return execute(source, writeMode);
    }
     ...
     * @return 路由后的 EventAttrRow 流
     */
    public static SingleOutputStreamOperator<EventAttrRow> execute(DataStream<ZGMessage> source, WriteMode writeMode) {
        // 1. 构建流水线
        SingleOutputStreamOperator<EventAttrRow> routed = buildDwPipeline(source);

        // 2. 添加 Sink
        addDorisSinks(routed, writeMode);
    ...
    /**
     * 添加 Doris Sink（使用配置中的写入模式）
     *
     * @param routed 路由后的数据流
     */
    public static void addDorisSinks(SingleOutputStreamOperator<EventAttrRow> routed) {
        addDorisSinks(routed, getWriteMode());
    }

    /**
     * 添加 Doris Sink (支持独立并行度和写入模式，避免反压)
     *
     * @param routed    路由后的数据流
     * @param writeMode 写入模式
     */
    public static void addDorisSinks(SingleOutputStreamOperator<EventAttrRow> routed, WriteMode writeMode) {
        ...
        // 使用指定的写入模式
        DorisSinkFactory.addEventAttrSink(routed, feNodes, database, username, password, callback, eventAttrParallelism, writeMode);
        DorisSinkFactory.addUserSink(userStream, feNodes, database, username, password, userParallelism, writeMode);
        DorisSinkFactory.addDeviceSink(deviceStream, feNodes, database, username, password, deviceParallelism, writeMode);
        DorisSinkFactory.addUserPropertySink(userPropStream, feNodes, database, username, password, callback, userPropParallelism, writeMode);

        LOG.info("Doris Sink 配置完成, feNodes={}, database={}, writeMode={}, parallelism: eventAttr={}, user={}, device={}, userProp={}",
                feNodes, database, writeMode, eventAttrParallelism, userParallelism, deviceParallelism, userPropParallelism);
    }

    // ==================== 内部构建方法 ====================

    /**
     * 获取写入模式（从配置读取，默认 STREAMING）
     */
    private static WriteMode getWriteMode() {
        String mode = Config.getString("doris.sink.write.mode", "STREAMING");
        try {
            return WriteMode.valueOf(mode.toUpperCase());
        } catch (IllegalArgumentException e) {
            LOG.warn("无效的写入模式配置: {}, 使用默认值 STREAMING", mode);
