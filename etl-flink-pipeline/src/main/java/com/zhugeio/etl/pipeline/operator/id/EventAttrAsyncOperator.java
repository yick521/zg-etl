...
public class EventAttrAsyncOperator extends RichAsyncFunction<ZGMessage, ZGMessage> {
    ...
    public void asyncInvoke(ZGMessage input, ResultFuture<ZGMessage> resultFuture) {
        ...
        try {
            ...
            processEventAttrs(input, appId, defaultOwner, dataList, resultFuture);
    ...
    private void processEventAttrs(ZGMessage input, Integer appId, String defaultOwner,
        ...
        batchGetOrCreateEventAttrs(allAttrs)
     ...
     * 按eventId分组处理，提高并发性能
     */
    private CompletableFuture<Map<String, AttrResult>> batchGetOrCreateEventAttrs(List<AttrInfo> attrs) {
        ...
        for (Map.Entry<Integer, List<AttrInfo>> entry : groupedByEvent.entrySet()) {
            ...
            CompletableFuture<Void> future = batchGetOrCreateForEvent(eventId, eventAttrs, results);
            futures.add(future);
        }
                    ...
                    .thenCompose(attrId -> {
                        if (attrId != null) {
                            ...
                            return cacheService.getEventAttrColumn(attr.getEventId(), attrId)
                                    .thenAccept(columnName -> results.put(attr.getKey(), new AttrResult(attrId, columnName)));
                        } else {
                            // 缓存未命中，需要创建
                            needCreate.add(attr);
                ...
                .thenCompose(v -> {
                    ...
                    return cacheService.isCreateAttrForbid(eventId)
                            .thenCompose(forbid -> {
                                ...
                                // 第三步：加锁批量创建
                                return batchCreateEventAttrsWithLock(eventId, needCreate, results);
                            });
                });
    }
        ...
        for (Map.Entry<String, List<AttrInfo>> entry : groupedByOwner.entrySet()) {
            ...
            CompletableFuture<Void> future = lockManager.executeWithLockAsync(
                    lockKey, LOCK_WAIT_MS, LOCK_EXPIRE_MS, () -> CompletableFuture.supplyAsync(() -> {
                        batchCreateEventAttrsInDb(eventId, owner, ownerAttrs, results);
                        return null;
                    })
            );
    ...
    private void batchCreateEventAttrsInDb(Integer eventId, String owner,
        ...
        try {
            ...
            // 第二步：获取下一个可用的列名（按owner独立管理，避免并发冲突）
     ...
     * <p>
     * 说明：
     * - 每个 eventId + owner 组合有独立的列号序列
     * - 例如：eventId=1, owner=A 可以有 cus1, cus2
     *        eventId=1, owner=B 也可以有 cus1, cus2（互不冲突）
     * - 由于已经按 eventId + owner 加锁，不会有并发冲突
     */
